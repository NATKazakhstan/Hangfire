            ...
            DateTime now)
        {
            using (connection.AcquireDistributedRecurringJobLock(recurringJobId, LockTimeout))
            {
                try
                {
                    ...
                    {
                        changedFields = recurringJob.GetChangedFields(out nextExecution);
                        var disabled = changedFields.ContainsKey("Disabled") && Boolean.TryParse(changedFields["Disabled"], out var parseDisabled) && parseDisabled;

                        if (nextExecution.HasValue && nextExecution <= now && !disabled)
                        {
